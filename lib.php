...
function zoom_calendar_item_update(stdClass $zoom) {
    ...
    if (!$zoom->recurring) {
        ...
        if (!empty($eventid)) {
            ...
            $event = populate_zoom_calender_item($zoom);
...
function zoom_create_recurring_weekly_events(stdClass $zoom) {
    ...
    $starttime = strtotime('Monday this week', $starttime);
...
function zoom_get_weekly_timestamp(int $weektimestamp, int $weekday) {
    ...
    $format = $weekdayoptions[$weekday] . ' this week';
...
function zoom_calendar_item_delete(stdClass $zoom) {
    ...
    // Delete all records associated with the zom instance.
    $zoomevents = $DB->get_records('event', [
        'modulename' => 'zoom',
        'instance' => $zoom->id
    ]);
    if ($zoomevents) {
        foreach ($zoomevents as $event) {
